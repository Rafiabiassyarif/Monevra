  ...
  return (
    <div className="min-h-screen bg-bg-dark text-slate-200 py-12 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        >
          <h1 className="text-3xl font-bold text-white mb-6">{t('legal.termsTitle')}</h1>
          <p className="text-slate-400 mb-8">{t('legal.lastUpdated')}</p>
          
          <div className="space-y-6 text-slate-300 leading-relaxed">
            {(t('legal.termsSections', { returnObjects: true }) as { title: string, content: string }[]).map((section, index) => (
              <section key={index}>
